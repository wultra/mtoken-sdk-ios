...
final class TOTPParserTest: XCTestCase {
    ...
    func testQRTOTPParserWithEmptyCode() {
        ...
        XCTAssertNil(WMTTOTPUtils.getTOTPFromQR(code: code))
    }
    ...
    func testQRTOTPParserWithValidCode() {
        ...
        XCTAssertEqual(WMTTOTPUtils.getTOTPFromQR(code: code)?.totp, "73743194", "Parsing of totp failed")
        XCTAssertEqual(WMTTOTPUtils.getTOTPFromQR(code: code)?.operationId, "6a1cb007-ff75-4f40-a21b-0b546f0f6cad", "Parsing of operationId failed")
    }

    
    func testDeeplinkTOTPParserWithInvalidURL() {
        let url = URL(string: "mtoken://an-invalid-url.com")!
        XCTAssertNil(WMTTOTPUtils.parseLoginDeeplink(url: url))
    }
    ...
    func testDeeplinkTOTPParserWithValidJWTCode() {
        ...
        XCTAssertEqual(WMTTOTPUtils.parseLoginDeeplink(url: url)?.totp, "56725494", "Parsing of totp failed")
        XCTAssertEqual(WMTTOTPUtils.parseLoginDeeplink(url: url)?.operationId, "df6128fc-ca51-44b7-befa-ca0e1408aa63", "Parsing of operationId failed")
    }
}
